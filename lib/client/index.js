...
window.WebSocket = new Proxy(window.WebSocket, {
  construct(target, args) {
    ...
    args[0] = (location.protocol=='https:' ? 'wss:' : 'ws:') + '//' + location.origin.split('/').splice(2).join('/') + pConfig.prefix + '?ws='+args[0].replace(location.origin.split('/').splice(2).join('/'), pLocation.origin.split('/').splice(2).join('/'))+ todo+'origin=' + new URL(pConfig.url).origin;
...
window._localStorage = new Proxy({}, {
  set(value, prop) {
    ...
    return localStorage[new URL(pConfig.url).hostname+prop] = value
  },
  get(value, prop) {
    if (prop=='getItem') {
      return function() {
        var args = arguments
        return localStorage[new URL(pConfig.url).hostname+args[0]]
      }
    }
    if (prop=='setItem') {
      return function() {
        var args = arguments
        return localStorage[new URL(pConfig.url).hostname+args[0]] = args[1]
      }
    }
    return localStorage[new URL(pConfig.url).hostname+prop]
  }
})
...
Object.keys(window.localStorage).forEach(key => {
  if (key.startsWith(new URL(pConfig.url).hostname)) {
...
window.pLocation = new Proxy({}, {
  set(obj, prop, value) {
      ...
      return location[prop] = new Base(ctx).url(new URL(pConfig.url).href.replace(new URL(pConfig.url)[prop], value));
  },
  get(obj, prop) {
      // Had to be done in order to fix Discord.
      if (new URL(pConfig.url).origin == atob('aHR0cHM6Ly9kaXNjb3JkLmNvbQ==') && new URL(pConfig.url).pathname == '/app') return window.location[prop];
      ...
      if (prop == 'assign' || prop == 'reload' || prop == 'replace' || prop == 'toString' || prop == 'hash' || prop == 'search') return {
          ...
          toString: () => { return new URL(pConfig.url).href },
          hash: window.location.hash,
          search: window.location.search
      }[prop];
      else return new URL(pConfig.url)[prop];
  }    
})
...
window.history.pushState = function() {
  arguments[2] = new Base(ctx).url(arguments[2])
  return pushstates.apply(this, arguments)
}
...
if (location.search && !(new URLSearchParams(location.search).get('palladium-redir'))) {
  var p1 = ctx.encoding.decode(location.pathname.split(pConfig.prefix)[1])
  console.log(p1+location.search+'&palladium-redir=true')
  location.href = pConfig.prefix+ctx.encoding.encode(p1+location.search+'&palladium-redir=true')
}
var proxify = {}
...
//if (new URL(pConfig.url).hostname===ctx.encoding.decode('hvtrs8%2F-dksaopd%2Ccmm').replace('https://', '')) myScript = {src: '//'}

...
module.exports = function Request(req, res, ctx) {
  ...
  if (!req.url.startsWith(ctx.prefix+`${ctx.encoding.encode(new URL(proxy.url).origin).replace('https:/', 'https:')}`)) return (res.writeHead(308, { location: ctx.prefix + `${ctx.encoding.encode(new URL(proxy.url).origin.replace('https:/', 'https:'))}`}), res.end(''));
  ...
  var requestMain = requestProtocol.request(ctx.url.replace(/pLocation/gi, 'location'), proxy.options, response => {
    ...
    response.on('data', (data) => {pData.push(data)}).on('end', () => {
      ...
      if (response.headers['set-cookie']) var cp = response.headers['set-cookie']; else var cp = undefined;
      ...
      if (cp) response.headers['set-cookie'] = cp

...
module.exports = class Smoke {
  ...
  ws(server) {
    new WebSocket.Server({server: server}).on('connection', (cli, req) => {
      ...
      const proxySocket = new WebSocket(proxyURL, protocol, options)
      
      const chunks = [];
  
      if (proxySocket.readyState == 0) cli.on('message', data => chunks.push(data));

      proxySocket.on('error', () => console.log('proxy error'))
      proxySocket.on('close', () => console.log('proxy close'))
      cli.on('error', () => console.log('client error'))
      cli.on('close', () => console.log('client close'))
